...
watch(search, (value) => {
  currentPage.value = 1;
  inertia.get("/en/home", { search: value, page: currentPage.value }, {
...
const loadMoreEpisodes = () => {
  ...
  currentPage.value++;
  inertia.get("/en/home", { page: currentPage.value, search: search.value }, {
      ...
      <div class="max-w-[1600px] mx-auto px-4 md:px-6 py-8 space-y-12">
        ...
        <section>
            <div class="flex flex-col items-center justify-between gap-4 mb-6 md:flex-row">
                ...
                <div class="relative w-full md:w-80">
                    ...
                    <input v-model="search" type="text" placeholder="Search..." class="w-full pr-10 pl-4 py-2.5 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333] rounded-full text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-4 gap-x-4 gap-y-8">
                <div 
                    ...
                    :key="episode.id"
                    @click="inertia.visit(`/en/episodes/${episode.id}`)"
                ...
                >
                    <div class="relative aspect-[16/10] w-full overflow-hidden rounded-xl bg-gray-200 dark:bg-[#1a1a1a] shadow-sm group-hover:shadow-md transition-all">
                        ...
                        <div v-if="index < 5" class="absolute top-0 right-0 z-20">
                            <span class="flex items-center gap-0.5 bg-red-600 text-white text-[9px] font-bold px-2 py-1 rounded-bl-lg shadow-md animate-pulse">
                                <Flame class="w-3 h-3 fill-current" /> New
                            </span>
                        </div>
                        ...
                        >
                             <div class="flex items-center justify-center text-white transition-all border rounded-full shadow-lg w-7 h-7 bg-black/50 backdrop-blur-md hover:bg-indigo-600 hover:scale-110 border-white/10" title="Episode Details">
                                 <Info class="w-4 h-4" />
                             </div>
                        </div>
                    ...
                    <div class="flex flex-col px-1">
                        <h3 class="text-sm font-bold text-gray-900 transition-colors dark:text-gray-100 line-clamp-1 group-hover:text-indigo-500">
                            {{ episode.series?.title || 'Title not available' }}
                        </h3>
                        <div class="flex items-center justify-between mt-1 text-[10px] text-gray-500 dark:text-gray-400">
                            <span class="truncate max-w-[70%] opacity-80">{{ episode.title || 'New Episode' }}</span>
        ...
        <section v-if="movies.length > 0">
            <div class="flex items-center justify-between mb-6">
                ...
                <Link href="/en/movies" class="flex items-center gap-1 text-sm font-bold text-indigo-500 hover:text-indigo-600">View All <ChevronLeft class="w-4 h-4" /></Link>
            </div>
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div 
                    v-for="movie in movies" 
                    :key="movie.id"
                    @click="inertia.visit(`/en/animes/${movie.id}`)"
                ...
                >
                    ...
                    <div class="flex flex-col flex-1 p-4">
                        ...
                        <p class="text-xs leading-relaxed text-gray-500 dark:text-gray-400 line-clamp-3">{{ movie.description || 'No description' }}</p>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section v-if="tvAnimes.length > 0">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Latest Animes</h2>
                </div>
                <Link href="/en/anime" class="flex items-center gap-1 text-sm font-bold text-indigo-500 hover:text-indigo-600">View All <ChevronLeft class="w-4 h-4" /></Link>
            </div>
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div 
                    v-for="anime in tvAnimes" 
                    :key="anime.id"
                    @click="inertia.visit(`/en/animes/${anime.id}`)"
                ...
                >
                    ...
                    <div class="flex flex-col flex-1 p-3">
                        ...
                        <div class="flex items-center gap-2 mb-2">
                             ...
                             <span class="text-[10px] text-gray-500">{{ anime.status || 'Ongoing' }}</span>
                        </div>
                        <p class="mb-auto text-xs leading-relaxed text-gray-500 dark:text-gray-400 line-clamp-2">{{ anime.description || 'No description' }}</p>
                    </div>
                </div>
            </div>
        </section>
                ...
                >
                    ...
                    <div class="p-5 pt-2" dir="rtl">
                        <div class="flex items-center justify-between mb-4 pb-4 border-b border-gray-100 dark:border-[#333]">
                             <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ selectedEpisode?.title || 'Episode Title' }}</h4>
                             <div class="flex items-center gap-3 text-xs text-gray-500">
                                <span class="flex items-center gap-1"><Eye class="w-3.5 h-3.5" /> {{ selectedEpisode?.views }}</span>
                                <span class="flex items-center gap-1"><Clock class="w-3.5 h-3.5" /> 24 min</span>
                             </div>
                        </div>

                        <p class="mb-6 text-xs leading-relaxed text-gray-600 dark:text-gray-400 line-clamp-4">
                            {{ selectedEpisode?.description || selectedEpisode?.series?.description || 'Watch this special episode and enjoy exciting events. Don’t miss watching it in the best quality.' }}
                        </p>
