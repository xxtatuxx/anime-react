...
const fetchFilteredAnimes = (pageUrl?: string) => {
  ...
  inertia.get(pageUrl || route('ar.anime'), {
    search: search.value,
...
<template>
  <Head title="قائمة الأنمي - AnimeLast" />
      ...
      <div class="max-w-[1600px] mx-auto px-4 md:px-6 py-8 space-y-8">
        ...
        <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">مسلسلات الأنمي</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">تصفح أحدث الحلقات والمسلسلات المستمرة</p>
                </div>
            </div>
        ...
        <div v-if="animes.length > 0" class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            ...
            >
                <Link :href="route('ar.animes.show', anime.id)" class="relative aspect-[2/3] overflow-hidden bg-gray-200 dark:bg-gray-800">
                    <img 
                        v-if="anime.image" 
        ...
        <div v-else-if="!loading" class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-900 dark:text-white">لم يتم العثور على أنميات</h3>
