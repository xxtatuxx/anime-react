...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Anime List', href: '/animes' },
    { title: props.anime.title || 'Details', href: `/animes/${props.anime.id}` },
];
...
const infoItems = computed(() => [
    { label: 'Status', value: props.anime.status, icon: null },
    { label: 'Type', value: props.anime.type, icon: Layers },
    { label: 'Season', value: props.anime.season?.name, icon: Calendar },
    { label: 'Studio', value: props.anime.studio_name, icon: Video },
    { label: 'Language', value: props.anime.language, icon: null },
    { label: 'Release Date', value: formattedReleaseDate.value, icon: Calendar },
]);
</script>

<template>
    <Head :title="props.anime.title || 'Anime Details'" />
    ...
    <AppLayout >
        <div class="flex flex-col flex-1 min-h-full gap-8 p-4 md:p-8">
                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="flex flex-col flex-1 p-6 lg:p-8">
                        ...
                        <p class="mb-6 text-base leading-relaxed text-gray-600 transition-all dark:text-gray-300 line-clamp-4 hover:line-clamp-none">
                            {{ props.anime.description || props.anime.description_en || 'No description available.' }}
                        </p>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6 lg:grid-cols-3 xl:grid-cols-4">
                            ...
                            <div class="p-3 border rounded-xl bg-gray-50 dark:bg-white/5 border-sidebar-border/40">
                                <span class="block mb-1 text-xs text-muted-foreground">Duration</span>
                                <div class="flex items-center gap-2">
                                    <Clock class="w-4 h-4 text-indigo-500" />
                                    <span class="text-sm font-semibold">{{ props.anime.duration ? `${props.anime.duration} min` : '-' }}</span>
                                </div>
                            </div>
                        </div>
            ...
            <div v-if="props.anime.episodes && props.anime.episodes.length > 0" class="space-y-4">
                <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div class="flex items-center gap-3">
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Episodes</h2>
                ...
                <div v-if="filteredEpisodes.length > 0" class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    <Link
                        ...
                        :key="episode.id"
                        :href="route('en.episodes.show', episode.id)"
                    ...
                    >
                        ...
                        <div class="p-3">
                            ...
                            <div class="flex items-center justify-between mt-2 text-xs text-muted-foreground">
                                ...
                                <span v-if="episode.created_at">{{ new Date(episode.created_at).toLocaleDateString('en-US') }}</span>
                            </div>
                        </div>
                    </Link>
                </div>
                ...
                <div v-else class="flex flex-col items-center justify-center py-12 text-center border border-dashed rounded-2xl border-sidebar-border/60">
                    ...
                    <p class="text-muted-foreground">No episodes match your search.</p>
                </div>
            </div>
            ...
            <div v-else class="flex flex-col items-center justify-center py-12 text-center border border-dashed bg-gray-50 dark:bg-white/5 rounded-3xl border-sidebar-border/60">
                ...
                <h3 class="text-lg font-semibold">No Episodes</h3>
                <p class="text-muted-foreground">No episodes have been added for this anime yet.</p>
            </div>
